// ============================================
// 🍽️ ITEM REPORT - Complete Implementation
// File: /public/js/pages/itemReport.js
// ============================================

export function initPage(branchId) {
  console.log("✅ Initializing Item Report with branchId:", branchId);
  ...
  function getReportData() {
    ...
    table.querySelectorAll("tbody tr").forEach((tr) => {
      ...
      if (cells.length > 0) {
        ...
        if (
          !firstCellText.includes("no item") &&
  ...
  function exportToCSV() {
    ...
    try {
      ...
      a.href = url;
      a.download = `item_report_${timestamp}.csv`;
      a.click();
  ...
  function exportToExcel() {
    ...
    try {
      ...
      const timestamp = new Date().toISOString().split("T")[0];
      const filename = `item_report_${timestamp}.xlsx`;
  ...
  async function exportToPDF() {
    ...
    try {
      ...
      // Generate filename
      const timestamp = new Date().toISOString().split("T")[0];
      const filename = `item_report_${timestamp}.pdf`;
      ...
      <div id="exportModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" style="animation: fadeIn 0.3s ease-out;">
        <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4" style="animation: slideDown 0.3s ease-out;">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-2xl font-bold text-gray-800">Export Item Report</h3>
          ...
          <p class="text-gray-600 mb-6">Choose export format for <span class="font-semibold text-blue-600">${reportData.data.length}</span> items:</p>
          
          <div class="space-y-3">
            <button onclick="window.exportItemReport('excel')" 
            ...
            <button onclick="window.exportItemReport('csv')" 
            ...
            <button onclick="window.exportItemReport('pdf')" 
  ...
  // Export handler
  window.exportItemReport = function (format) {
  ...
  console.log("✅ Item Report initialized successfully");
}
