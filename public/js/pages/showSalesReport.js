// ============================================
// 📊 SALES REPORT - Complete Implementation
// File: /public/js/pages/salesReport.js
// ============================================

export function initPage(branchId) {
  console.log("✅ Initializing Sales Report with branchId:", branchId);
  ...
  function getReportData() {
    ...
    table.querySelectorAll("tbody tr").forEach((tr) => {
      ...
      if (cells.length > 0) {
        ...
        if (
          !firstCellText.includes("no sales") &&
  ...
  function getMetadata() {
    return {
      ...
      endDate:
        ...
        "",
      totalQuantity:
        document.querySelector("[data-total-quantity]")?.textContent || "0",
      totalAmount:
        document.querySelector("[data-total-amount]")?.textContent || "₹0",
    };
  }
  ...
  function exportToCSV() {
    ...
    try {
      ...
      // Add totals
      csv.push([]);
      csv.push(["Total Quantity", metadata.totalQuantity]);
      csv.push(["Total Amount", metadata.totalAmount]);
  ...
  async function exportToPDF() {
    ...
    try {
      ...
      doc.setFontSize(10);
      doc.text(`Total Quantity: ${metadata.totalQuantity}`, 14, finalY);
      doc.text(`Total Amount: ${metadata.totalAmount}`, 14, finalY + 6);
      ...
      <div id="exportModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" style="animation: fadeIn 0.3s ease-out;">
        <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4" style="animation: slideDown 0.3s ease-out;">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-2xl font-bold text-gray-800">Export Sales Report</h3>
          ...
          <p class="text-gray-600 mb-6">Choose export format for <span class="font-semibold text-blue-600">${reportData.data.length}</span> records:</p>
          
          <div class="space-y-3">
            <button onclick="window.exportSalesReport('excel')" 
            ...
            <button onclick="window.exportSalesReport('csv')" 
            ...
            <button onclick="window.exportSalesReport('pdf')" 
